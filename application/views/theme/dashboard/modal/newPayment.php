...
?><div class="modal-dialog" role="document">
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title" id="newEventMoalLebel"><strong><?= getState($_currentState) ?></strong> New Payment</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
            </button>
        </div>
        <div class="modal-body">
            <form method="post" id="newPaymentForm" novalidate action="<?= dashboard_url("newPayment/" . $_currentState) ?>" class="container-fluid">
                <div class="row">
                    <div class="col-12">
                    ...
                    <div class="col-6">
                        <fieldset class="form-group">
                            ...
                            <input class="form-control" name="price" id="price" value="<?= strlen($_currentState) === 3 ? "84" : ($_currentState === "CA" ? "125" : "94") ?>" type="text" required>
                            <p class="help-block m-0 danger"></p>
                        </fieldset>
                    </div>  
...
?>
    ...
    $("#customer").select2({
        ...
        tags: [],
        ajax: {
            url: '<?= dashboard_url("getCustomerList") ?>',
            dataType: 'json',
            type: "POST",
            quietMillis: 00,
            data: function (term) {
    ...
    /*var todayPicker = $(".todayDate").pickadate({
     ...
     console.log(todayPicker);
     if (todayPicker.pickadate('picker')) {
     
     todayPicker.pickadate('picker').set("select", new Date(Date.now()));
     }
     update prospects 
     set prospects.ordered = '1'
     where prospects.contactID in (select orders.contactID from orders)
     **/

...
if (isset($_PG["id_check"])) {
    ...
    if ($id_check == 0) {
        $idbi = $_PG["id_bank_info"];
        $sql = "SELECT MAX(check_number) AS check_number FROM accounting_checks WHERE id_bank_info=$idbi";
        //die($sql);
        ...
        if ($row["check_number"] != "") {
            ...
            $sql = "SELECT accounting_bank_info.* FROM accounting_bank_info WHERE accounting_bank_info.id_bank_info=$idbi";
            $result = $conn->query($sql);
            $row = $result->fetch_assoc();
            $check_number = $row['check_number_start'];
        }
        //die($check_number);
        $query = "INSERT INTO accounting_checks (id_bank_info, check_number, check_date, payee, check_amount, check_amount_letter, payee_name, payee_address1, payee_address2, payee_address3, memo, note1, note2, date_created, id_account) VALUES ('" . $idbi . "', '" . $check_number . "', CURDATE(), NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NOW(), '" . $_SESSION["id_user"] . "')";
        //die($query);
